...
lr = 0.001 # Learning rate for use of constant learnig rate
...
def main():
    if INIT_NUM == -1:
        player = DQNAgent(id=1, path=path_load, train=True) # The agent we're training
    else: # If there is a valid init path
        player = DQNAgent(id=1, path=init_path, train=True)
    buffer = ReplayBuffer(path=buffer_path, batch_size=batch_size) # The buffer
    ...
    for epoch in range(start, epochs):
        ...
        while not state.is_game_over:
            ...
            r1, done = as1.get_reward()

            if done: # If end of game
                res += r1
                buffer.push(as1, r1, done, as1)
                state = as1
                if as1.game_winner == 1:
                    score += 1
                break
            if state2: # If not first move
                buffer.push(as2, r1-r2, done, as1)
            
            # If not end of game
            # Get O action
            state2 = as1.copy()
            action= player.get_action(state2, epoch, True)
            as2 = state2.copy()
            as2.make_move(*action)
            
            r2, done = as2.get_reward()
                
            if done: # If end of game
                res += r2
                buffer.push(as2, -r2, done, as2)
                if as2.game_winner == 2:
                    score -= 1
                break
            buffer.push(as1, r1-r2, done, as2)
            state = as2
            ...
            if len(buffer) >= min_buffer:
                ...
                Q_values = Q(states) # Get Q values
                with tr.inference_mode(): # Get Q_hat values
